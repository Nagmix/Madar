...
/// NestJS API Client - Complete integration layer with NestJS backend
/// 
...
class NestjsApiClient {
  ...
  /// Register new user - NestJS Auth Module
  /// POST /auth/register
  ...
  /// Get user profile - NestJS User Module
  /// GET /users/profile
  ...
  /// Update user profile - NestJS User Module
  /// PUT /users/profile
  ...
  /// Create a new trip - NestJS Trip Module
  /// POST /trips
  ...
  /// Get nearby drivers - NestJS Dispatch Module (PostGIS query)
  /// GET /dispatch/nearby-drivers
  Future<List<DriverModel>> getNearbyDrivers({
    ...
    return items.map((e) => DriverModel.fromJson(e as Map<String, dynamic>)).toList();
  }

  /// Request a ride - NestJS Dispatch Module
  ...
  Future<DispatchResult> requestRide(CreateTripRequest request) async {
    ...
    return DispatchResult.fromJson(response.data);
  }
